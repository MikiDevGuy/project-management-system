...
$phase_id = $_GET['id'] ?? null;

if (!$phase_id) {
    die('<div class="alert alert-warning">Phase ID not provided.</div>');
}

// Check if the phase exists and fetch its details
$phase_stmt = $conn->prepare("SELECT p.*, pr.name as project_name FROM phases p JOIN projects pr ON p.project_id = pr.id WHERE p.id=?");
$phase_stmt->bind_param("i", $phase_id);
$phase_stmt->execute();
$phase_result = $phase_stmt->get_result();

if ($phase_result->num_rows === 0) {
    die('<div class="alert alert-warning">Phase not found.</div>');
}

$phase = $phase_result->fetch_assoc();

// Fetch currently assigned user IDs for this phase
$assigned_ids = [];
$assigned_stmt = $conn->prepare("SELECT user_id FROM phase_users WHERE phase_id = ?");
$assigned_stmt->bind_param("i", $phase_id);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['assigned_users'])) {
    ...
    // Clear all existing assignments for this phase first
    $clear_stmt = $conn->prepare("DELETE FROM phase_users WHERE phase_id = ?");
    $clear_stmt->bind_param("i", $phase_id);
    $clear_stmt->execute();
    ...
    foreach ($assigned_users as $user_id) {
        $stmt = $conn->prepare("INSERT INTO phase_users (phase_id, user_id) VALUES (?, ?)");
...
<head>
  ...
  <title>Assign Users to Phase</title>
  <!-- Bootstrap 5 CSS -->
...
<body>
  ...
  <div class="container">
    ...
    <a href="#" onclick="history.back(); return false;" class="back-link mb-3 d-inline-block">
      <i class="fas fa-arrow-left me-2"></i>Back to All Phases
    </a>

    <!-- Hero Section -->
    <div class="hero-section">
      <h1><i class="fas fa-user-plus me-2"></i>Assign Users</h1>
      <p class="lead mb-0">Phase: <?= htmlspecialchars($phase['name']) ?> (Project: <?= htmlspecialchars($phase['project_name']) ?>)</p>
    </div>
    ...
    <?php if (!empty($newly_assigned)): ?>
      <div class="alert alert-success">
        ...
        Successfully assigned to phase: <?= implode(", ", $newly_assigned) ?>
      </div>
    <?php endif; ?>
        ...
        <div class="card card-custom mb-4">
          ...
          <div class="card-body">
            ...
            $assigned = $conn->query("
                ...
                JOIN phase_users pu ON u.id = pu.user_id
                WHERE pu.phase_id = $phase_id
