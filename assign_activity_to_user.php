...
// Check if the activity exists and fetch its details
$activity_stmt = $conn->prepare("
    SELECT a.*, p.name as phase_name, pr.name as project_name 
    FROM activities a 
...
if ($activity_result->num_rows === 0) {
    die('<div class="alert alert-warning">Activity not found.</div>');
}

$activity = $activity_result->fetch_assoc();

// Fetch currently assigned user IDs for this activity
$assigned_ids = [];
$assigned_stmt = $conn->prepare("SELECT user_id FROM activity_users WHERE activity_id = ?");
$assigned_stmt->bind_param("i", $activity_id);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['assigned_users'])) {
    ...
    // Clear all existing assignments for this activity first
    $clear_stmt = $conn->prepare("DELETE FROM activity_users WHERE activity_id = ?");
    $clear_stmt->bind_param("i", $activity_id);
    $clear_stmt->execute();
    ...
    foreach ($assigned_users as $user_id) {
        $stmt = $conn->prepare("INSERT INTO activity_users (activity_id, user_id) VALUES (?, ?)");
...
<head>
  ...
  <title>Assign Users to Activity</title>
  <!-- Bootstrap 5 CSS -->
...
<body>
  ...
  <div class="container">
    ...
    <a href="view_assigned_activities.php?id=<?= $activity['phase_id'] ?>" class="back-link mb-3 d-inline-block">
      <i class="fas fa-arrow-left me-2"></i>Back to All Activities
    </a>

    <!-- Hero Section -->
    <div class="hero-section">
      <h1><i class="fas fa-user-plus me-2"></i>Assign Users</h1>
      <p class="lead mb-0">Activity: <?= htmlspecialchars($activity['name']) ?> (Phase: <?= htmlspecialchars($activity['phase_name']) ?>)</p>
    </div>
    ...
    <?php if (!empty($newly_assigned)): ?>
      <div class="alert alert-success">
        ...
        Successfully assigned to activity: <?= implode(", ", $newly_assigned) ?>
      </div>
    <?php endif; ?>
        ...
        <div class="card card-custom mb-4">
          ...
          <div class="card-body">
            ...
            $assigned = $conn->query("
                ...
                JOIN activity_users au ON u.id = au.user_id
