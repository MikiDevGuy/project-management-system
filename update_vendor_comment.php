...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $current_username = $_SESSION['username'] ?? 'Unknown Vendor';
    ...
    try {
        if ($action === 'update_comment') {
            ...
            $new_vendor_comment = $_POST['vendor_comment'] ?? null;

            if (!$test_case_id || $new_vendor_comment === null) {
                throw new Exception("Invalid request parameters for update_comment.");
            }

            // 1. Update test_cases table
            $update_stmt = $conn->prepare("UPDATE test_cases SET vendor_comment = ?, updated_at = NOW() WHERE id = ?");
            $update_stmt->bind_param("si", $new_vendor_comment, $test_case_id);
            $update_stmt->execute();
            ...
            if ($update_stmt->affected_rows > 0) {
                ...
                $log_action = 'Vendor Comment Updated';
                $log_description = "Vendor updated comment: " . $new_vendor_comment;
                ...
                $get_recipients_stmt = $conn->prepare("
                    ...
                    WHERE (u.system_role = 'super_admin' OR (u.system_role = 'tester' AND pu.project_id = ?))
                ...
                $response['message'] = "Comment updated and logged successfully. Ready to send notifications.";
                ...
                $response['new_vendor_comment'] = $new_vendor_comment; // Pass back for email body

            } else {
                $response['message'] = "No changes made to vendor comment or test case not found.";
            }
            ...
            $commenting_username = $_POST['commenting_username'] ?? 'Unknown Vendor';
            $vendor_comment_body = $_POST['vendor_comment_body'] ?? '';
            ...
            $email_subject = "New Vendor Comment on Test Case: " . htmlspecialchars($test_case_title);
            $email_body = "
                <p>Hello,</p>
                <p><strong>" . htmlspecialchars($commenting_username) . "</strong> has updated a vendor comment on the test case: <strong>" . htmlspecialchars($test_case_title) . "</strong> (Project ID: " . htmlspecialchars($project_id) . ").</p>
                <p><strong>Comment:</strong><br>" . nl2br(htmlspecialchars($vendor_comment_body)) . "</p>
                <p>You can view the test case here: <a href='http://yourdomain.com/view_project.php?project_id=" . $project_id . "&test_case_id=" . $test_case_id . "'>View Test Case</a></p>
